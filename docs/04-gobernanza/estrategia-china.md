...
```mermaid
flowchart LR
    A["CDB / Eximbank<br/>Desembolsa USD"] --> B["BANDES / FONDEN<br/>(Venezuela)"]
    B --> C["Gasto fiscal<br/>(no invertido en PDVSA)"]
    D["PDVSA<br/>Produce crudo"] --> E["CNPC<br/>(Refinerías en China)"]
    E -->|"Precio con descuento"| F["Pago se descuenta<br/>de la deuda"]
...
```mermaid
flowchart TD
    A["Licencia 46B activa<br/>(China excluida de petróleo)"] --> B{¿Sanciones se relajan<br/>para China?}
    B -->|"No (probable 3-5 años)"| C["Negociar deuda en<br/>instrumentos NO petroleros:<br/>gas, bonos, forward parcial"]
    B -->|"Sí (post-normalización)"| D["Forward contracts de crudo<br/>a China viables<br/>(max 15% producción)"]
    C --> E["Debt-for-climate swap<br/>Gas LNG contracts<br/>Bonos reestructurados"]
    D --> F["China como UN comprador<br/>entre 5+ diversificados"]
...
```mermaid
pie title "Reestructuración propuesta de USD 16-19B de deuda china"
    "Forward contracts GNL (15%)" : 6
    "Equity minoritario gas (30% max)" : 4
    "Debt-for-climate swap" : 4
    "Bonos reestructurados (haircut 40%)" : 5
```
...
```mermaid
gantt
    title Reestructuración Deuda China — Timeline Operativo
    dateFormat YYYY
    axisFormat %Y

    section Fase 1: Auditoría
    Publicar contratos 2007-2019                  :a1, 2027, 2028
    Cuantificar deuda exacta                      :a2, 2027, 2028
    Contratar asesores especializados              :a3, 2027, 2028

    section Fase 2: Señalización
    Canal diplomático presidencial con Beijing     :b1, 2028, 2029
    Propuesta marco presentada                     :b2, 2028, 2029
    Negociación FMI en paralelo                    :b3, 2028, 2030

    section Fase 3: Negociación técnica
    Forward contracts GNL                          :c1, 2029, 2030
    Equity minoritario gas                         :c2, 2029, 2030
    Debt-for-climate swap                          :c3, 2029, 2031
    Bonos reestructurados                          :c4, 2030, 2031

    section Fase 4: Ejecución
    Firma de acuerdos definitivos                  :d1, 2030, 2031
    Primeras entregas GNL                          :d2, 2031, 2032

    section Fase 5: Normalización
    Relación comercial estándar                    :e1, 2031, 2033
```
...
```mermaid
flowchart TD
    A["Relación con EE.UU."] --> B["Petróleo (100%)"]
    A --> C["Seguridad + Inteligencia"]
    A --> D["Tech / Data Centers / 5G"]
    A --> E["Sistema financiero"]

    F["Relación con China"] --> G["Gas natural (max 15%)"]
    F --> H["Compra de minerales procesados"]
    F --> I["Bienes de consumo / manufactura"]
    F --> J["Debt-for-climate swaps"]

    K["Neutral / Competitivo"] --> L["Infraestructura (licitación abierta)"]
    K --> M["Turismo"]
    K --> N["Agricultura de exportación"]
