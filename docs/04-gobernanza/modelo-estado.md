...
flowchart TD
    ...
    STATE --> S1["⚙️ Gobierno<br/>Administración mínima"]
    STATE --> S2["🏥 Salud<br/>Cobertura universal"]
    STATE --> S3["⚖️ Justicia<br/>Tribunales independientes"]
    STATE --> S4["📚 Educación<br/>Pública de calidad"]
    STATE --> S5["🔒 Seguridad<br/>Policía profesional"]
    PRIV["🏗️ Sector Privado"] --> I1["Infraestructura<br/>Concesiones"]
    PRIV --> I2["Pensiones<br/>Cuentas individuales"]
    PRIV --> I3["Salud complementaria<br/>Seguros privados"]
    PRIV --> I4["Telecomunicaciones"]
    PRIV --> I5["Servicios públicos<br/>Agua, electricidad"]

    style OIL fill:#0D47A1,color:#fff
    style VSA fill:#0D47A1,color:#fff
    style TAX fill:#00897B,color:#fff
    style STATE fill:#F9A825,color:#000
    style FUND fill:#0D47A1,color:#fff
    style PRIV fill:#E3F2FD,color:#1B2631
...
| **Cesantía** *(Mochila Austriaca)* | 2% | Colchón de 3-6 meses de salario si pierdes el empleo. **Portátil**: la cuenta viaja contigo de empleador en empleador. Si renuncias o te despiden, el saldo es tuyo. Si no la usas → se transfiere a Retiro al jubilarte. Elimina el desincentivo a contratar que genera la indemnización tradicional. | Del trabajador — disponible inmediato si queda desempleado | [Austria BMSVG 2003](https://www.usp.gv.at/en/themen/mitarbeiter-und-gesundheit/beendigung-des-arbeitsverhaeltnisses/weitere-informationen-beendigung/abfertigung-neu-fuer-arbeitnehmer-und-freie-dienstnehmer.html) (1,53%/mes desde 2003) · [Chile AFC](https://www.afc.cl/) · [Argentina FAL — Ley 27.802, 2026](https://www.batimes.com.ar/news/economy/explainer-key-points-of-mileis-newly-passed-labour-reform-package.phtml) (1-2,5% según tamaño empresa) |
...
flowchart TB
    ...
    TAX --> P1["👴 Pensión Pilar 1<br/>Universal para todo jubilado"]
    TAX --> SOL["❤️ Solidaridad<br/>Salud Tramo A/B + menores"]

    style FCV fill:#0D47A1,color:#fff
    style RET fill:#F9A825,color:#000
    ...
    style SOL fill:#E3F2FD,color:#1B2631
```
...
```mermaid
flowchart LR
    F1["Años 1-5<br/>FONASA 100%<br/>Solidaridad pura<br/>Nadie queda fuera"] --> F2["Años 5-10<br/>FONASA 5% + Medisave 2%<br/>Se introduce ahorro personal"]
    F2 --> F3["Años 10-15<br/>FONASA 4% + Medisave 3%<br/>Clase media ahorra"]
    F3 --> F4["Años 15+<br/>FONASA 3% + Medisave 4%<br/>Híbrido Chile-Singapur"]
    style F1 fill:#C62828,color:#fff
    style F2 fill:#F9A825,color:#000
    style F3 fill:#0D47A1,color:#fff
    style F4 fill:#00897B,color:#fff
```
...
| **Colegios públicos → empresas privadas** | Instalaciones actuales se entregan en **concesión o comodato** (no se regalan). Deben mantenerlas en óptimas condiciones. El apoyo de modernización es un **crédito con condiciones ventajosas** (no subsidio). Cuando puedan, adquieren el inmueble. Autonomía de gestión total | [Chile: colegios subvencionados](https://www.mineduc.cl/) — compiten por vouchers |
...
| **Acreditación ciudadana (app)** | Cero burocracia, cero inspector corrupto. **App pública** donde padres, estudiantes y comunidad reportan en tiempo real: estado de instalaciones, calidad de comida, seguridad, limpieza, calidad docente. Fotos + rating + comentarios. Si el rating baja de umbral → alerta automática → plazo para reparar → si no cumple, pierde acreditación. **Anti-boicot y anti-bullying:** solo pueden evaluar usuarios verificados (padres con hijos matriculados), IA detecta patrones de reseñas coordinadas/falsas, el evaluado puede apelar ante comité independiente, las reseñas se ponderan por antigüedad y consistencia (no vale 50 reviews negativos el mismo día), identidad del evaluador protegida pero verificable ante apelación | Modelo: [Google Reviews](https://maps.google.com/) + [Airbnb Trust & Safety](https://www.airbnb.com/trust) |
| **Evaluación de profesores** | Profesores evaluados por resultados + encuesta de padres/estudiantes en la app. Incentivos por desempeño: bonos anuales para los mejor evaluados | [Singapur: Enhanced Performance Management System](https://www.moe.gov.sg/) |
...
```mermaid
flowchart TB
    V["💰 VOUCHER UNIVERSAL<br/>Sigue al estudiante — no al colegio"]
    V --> C1["🏫 Colegio Privado<br/>Tradicional"]
    V --> C2["🏢 Colegio Público<br/>→ Empresa Autónoma"]
    V --> C3["💻 Colegio en Línea<br/>+ Red de Tutores"]
    V --> C4["🎵 Actividades<br/>Idiomas, música, artes,<br/>deportes, robótica"]

    AUD["📋 AUDITORÍA + EVALUACIÓN<br/>Colegios auditados por resultados<br/>Profesores evaluados periódicamente<br/>Bonos por desempeño"]
    ...
    style AUD fill:#C62828,color:#fff
```
...
```mermaid
flowchart TB
    UNI["🎓 UNIVERSIDAD PÚBLICA<br/>Autónoma + Autosostenible"]
    UNI --> V["💰 Vouchers por estudiante<br/>(se ganan por mérito)"]
    UNI --> ID["🔬 I+D + Patentes<br/>Licencias → royalties"]
    UNI --> AL["🤝 Alianzas Privadas<br/>Cátedras + laboratorios"]
    UNI --> SP["🚀 Spin-offs<br/>Incubadoras → startups"]
    UNI --> CO["📋 Consultoría<br/>Servicios técnicos"]
    UNI --> EC["📚 Educación Continua<br/>Maestrías + certificaciones"]

    EST["👤 ESTUDIANTE"]
    EST -->|"Mérito: notas + admisión"| V
    EST -->|"Salud FONASA automática"| SAL["🏥 Salud cubierta"]
    EST -->|"Si Tramo A/B"| ALIM["🍽️ Alimentación"]

    SUP["📋 SUPERVISIÓN TRIPARTITA<br/>Estado + Comunidad Académica + Estudiantes"]
    UNI --> SUP

    style UNI fill:#0D47A1,color:#fff
    style V fill:#F9A825,color:#000
    style EST fill:#00897B,color:#fff
    style SUP fill:#C62828,color:#fff
    style SAL fill:#00897B,color:#fff
    style ALIM fill:#00897B,color:#fff
    style ID fill:#E3F2FD,color:#1B2631
    style AL fill:#E3F2FD,color:#1B2631
    style SP fill:#E3F2FD,color:#1B2631
    style CO fill:#E3F2FD,color:#1B2631
    style EC fill:#E3F2FD,color:#1B2631
```
...
```mermaid
xychart-beta
    title "Fuentes de financiamiento por fase (USD miles de M)"
    x-axis ["Año 1", "Año 3", "Año 7", "Año 15"]
...
```mermaid
xychart-beta
    title "FCV acumulado por edad — Salario mínimo (USD miles)"
    x-axis ["18", "25", "30", "35", "40", "50", "60", "65"]
    y-axis "USD miles" 0 --> 500
    bar [20, 37, 57, 72, 108, 205, 368, 464]
```
