---
sidebar_position: 17
title: "Aviación Comercial y Aeropuertos"
---
...
```mermaid
flowchart TD
    subgraph DEMANDA["DEMANDA LATENTE"]
        D1["Diáspora 7.9M<br/>Rutas a Santiago, Lima,<br/>Buenos Aires, Miami, Madrid"]
        D2["Turismo receptor<br/>Canaima, Los Roques,<br/>Mérida, Margarita"]
        D3["Negocios + inversión<br/>Petróleo, minería,<br/>data centers, tech"]
        D4["Tráfico doméstico<br/>30M habitantes<br/>sin alternativa terrestre viable"]
    end

    subgraph CUELLO["CUELLO DE BOTELLA"]
        C1["Aeropuertos degradados"]
        C2["Solo 1 aerolínea estatal"]
        C3["6 internacionales retiradas"]
        C4["Sin low-cost carriers"]
    end

    subgraph SOLUCION["SOLUCIÓN"]
        S1["Concesiones aeroportuarias<br/>Fraport, Vinci, GAP"]
        S2["Nuevas aerolíneas<br/>Low-cost + full-service"]
        S3["Open skies policy<br/>Copa, JetBlue, LATAM, Spirit"]
        S4["Hub de carga<br/>Flores, cacao, oro, seafood"]
    end

    DEMANDA --> CUELLO
    CUELLO -->|"USD 5-9B<br/>inversión"| SOLUCION

    style DEMANDA fill:#0D47A1,color:#fff
    style CUELLO fill:#C62828,color:#fff
    style SOLUCION fill:#00897B,color:#fff
```
...
```mermaid
xychart-beta
    title "Inversión por componente (USD millones — rango inferior)"
    x-axis ["Terminales pax", "Pistas", "Cargo", "MRO", "ATC + nav", "Seguridad", "Acceso", "Combustible"]
    y-axis "USD millones" 0 --> 2500
    bar [1500, 600, 400, 300, 200, 150, 500, 100]
```
...
```mermaid
xychart-beta
    title "Pasajeros aéreos — proyección (millones/año)"
    x-axis ["Actual", "Año 1", "Año 3", "Año 5", "Año 7", "Año 10"]
    y-axis "Millones de pasajeros" 0 --> 35
    bar [2, 4, 10, 18, 25, 32]
```
