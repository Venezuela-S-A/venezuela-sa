...
flowchart TB
    ...
    subgraph FIN["VERTICALES FINTECH"]
        NEO["1. Neo-Banking<br/>Cuentas digitales USD"]
        PAG["2. Pagos Instantáneos<br/>P2P + comercios"]
        REM["3. Remesas<br/>USD 3-5B/ano"]
        CRED["4. Micro-Crédito<br/>+ Lending"]
        SEG["5. Insurtech<br/>Seguros digitales"]
        INV["6. Wealth / Ahorro<br/>Inversión ciudadana"]
    end

    CORE --> FIN

    style CORE fill:#0D47A1,color:#fff
    style NEO fill:#00897B,color:#fff
    style PAG fill:#00897B,color:#fff
    style REM fill:#F9A825,color:#000
    style CRED fill:#0D47A1,color:#fff
    style SEG fill:#0D47A1,color:#fff
...
```mermaid
gantt
    title FinTech Venezuela — Timeline de Implementación
    dateFormat YYYY
    axisFormat %Y

    section Sprint 1: Regulación + Rieles (Meses 1-12)
    Sandbox regulatorio fintech           :s1a, 2027, 270d
    Identidad digital + KYC APIs          :s1b, 2027, 365d
    Corredor OFAC para pagos              :s1c, 2027, 365d
    Licencia Visa/Mastercard              :s1d, 2027, 270d

    section Sprint 2: Primeros Productos (Meses 6-18)
    Neo-banco piloto 1M usuarios          :s2a, 2027, 365d
    Sistema pagos instantáneos (Pix-VE)   :s2b, 2027, 365d
    Remesas digitales operativas          :s2c, 2028, 270d
    Micro-crédito piloto                  :s2d, 2028, 270d

    section Fase 2: Escala (Ano 2-5)
    10M+ usuarios activos                 :f2a, 2028, 730d
    Lending a escala                      :f2b, 2029, 730d
    Insurtech lanzamiento                 :f2c, 2029, 730d
    Wealth y ahorro digital               :f2d, 2029, 730d
    Inversión ciudadana via app           :f2e, 2030, 365d

    section Fase 3: Hub Regional (Ano 5-10)
    30M+ usuarios activos                 :f3a, 2031, 1825d
    Exportar modelo a Caribe              :f3b, 2032, 1095d
    Venezuela como hub fintech LATAM      :f3c, 2033, 1095d
```
...
```mermaid
xychart-beta
    title "Revenue FinTech Venezuela (USD millones)"
    x-axis ["Ano 1", "Ano 3", "Ano 5", "Ano 7", "Ano 10"]
    y-axis "USD millones" 0 --> 5000
    bar [65, 580, 1800, 3000, 4300]
```
...
```mermaid
flowchart LR
    FT["FinTech<br/>Infraestructura"] --> TAX["Recaudacion Fiscal<br/><b>+USD 1-3B IVA/ano</b><br/>al formalizar economía"]
    FT --> REM2["Remesas Baratas<br/><b>USD 280M/ano</b><br/>liberados para familias"]
    FT --> CRED2["Crédito PYME<br/><b>500K+ negocios</b><br/>con acceso a capital"]
    FT --> INV2["Inversión Ciudadana<br/><b>Bonos VIN via app</b><br/>40M accionistas"]
    FT --> TECH["Talento Tech<br/><b>40K+ ingenieros</b><br/>formados por demanda"]

    style FT fill:#00897B,color:#fff
    style TAX fill:#F9A825,color:#000
    style REM2 fill:#F9A825,color:#000
    style CRED2 fill:#0D47A1,color:#fff
    style INV2 fill:#0D47A1,color:#fff
    style TECH fill:#0D47A1,color:#fff
```
