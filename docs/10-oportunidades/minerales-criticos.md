...
```mermaid
pie title Dominio chino en minerales críticos (% producción global, 2024)
    "China" : 63
    "Resto del mundo" : 37
```
...
```mermaid
flowchart TD
    A["ARCO MINERO<br>111.843 km²"] --> B["FANB<br>Cobra acceso + combustible"]
    A --> C["ELN<br>Concesiones ilegales de Maduro"]
    A --> D["Tren de Aragua<br>Sindicato minero en Las Claritas"]
    A --> E["Pranes locales<br>Control de minas artesanales"]

    B --> F["Extracción ilegal<br>~USD 2.200 M/año en oro"]
    C --> F
    D --> F
    E --> F

    F --> G["Contrabando a<br>Colombia, Guyana, Brasil"]
    G --> H["Mercado internacional<br>Oro 'lavado' como legal"]
...
```mermaid
flowchart LR
    A["Bauxita<br>6.000 M ton"] --> B["Alumina<br>Refinería"]
    B --> C["Aluminio<br>Fundición con hidro"]
    C --> D["Galio<br>Subproducto"]
    D --> E["Semiconductores<br>Exportación a EE.UU."]

    F["Coltán"] --> G["Tantalio<br>Refinación"]
    F --> H["Niobio<br>Ferro-niobio"]
    G --> I["Capacitores<br>Electrónica"]
    H --> J["Acero especial<br>Aeroespacial"]

    K["Hierro<br>18.000 M ton"] --> L["DRI/HBI<br>Reducción directa"]
    L --> M["Acero verde<br>Con hidro-electricidad"]
...
```mermaid
flowchart TD
    subgraph ENERGIA["Energía"]
        E1["Líneas transmisión<br>Guri → Arco Minero<br>USD 500-800 M"]
        E2["Subestaciones<br>5-10 nuevas<br>USD 200-400 M"]
    end

    subgraph TRANSPORTE["Transporte"]
        T1["Carreteras<br>500+ km nuevas<br>USD 1.000-2.000 M"]
        T2["Ferrocarril<br>Rehabilitación CVG<br>USD 800-1.500 M"]
        T3["Puertos fluviales<br>Orinoco<br>USD 300-600 M"]
    end

    subgraph PROCESAMIENTO["Plantas"]
        P1["Refinería alumina<br>USD 1.500-2.500 M"]
        P2["Planta tantalio/niobio<br>USD 300-600 M"]
        P3["Planta DRI/HBI<br>USD 1.500-3.000 M"]
    end

    subgraph SOPORTE["Soporte"]
        S1["Campamentos<br>+ ciudades mineras<br>USD 500-1.000 M"]
        S2["Centro de datos<br>geológicos<br>USD 50-100 M"]
        S3["Hospital regional<br>+ escuelas<br>USD 200-400 M"]
    end

    ENERGIA --> PROCESAMIENTO
    TRANSPORTE --> PROCESAMIENTO
    SOPORTE --> PROCESAMIENTO

    style ENERGIA fill:#F9A825,color:#000
    style TRANSPORTE fill:#0D47A1,color:#fff
    style PROCESAMIENTO fill:#E3F2FD,color:#1B2631
    style SOPORTE fill:#00897B,color:#fff
```
...
```mermaid
flowchart LR
    A["1. Control<br>territorial"] --> B["2. Desmantelar<br>redes criminales"]
    B --> C["3. Marco legal<br>+ regulador"]
    C --> D["4. Survey<br>geológico"]
    D --> E["5. Licitación<br>internacional"]
    E --> F["6. Operación<br>minera"]
...
```mermaid
xychart-beta
    title "Ingresos mineros proyectados (USD M/año — escenario conservador)"
    x-axis ["Año 1", "Año 2", "Año 3", "Año 5", "Año 7", "Año 10"]
    y-axis "USD millones" 0 --> 12000
    bar [300, 800, 2500, 5000, 8000, 11500]
    line [300, 800, 2500, 5000, 8000, 11500]
```
