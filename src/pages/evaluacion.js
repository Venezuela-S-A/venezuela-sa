...
function scoreColor(score) {
  if (score === null || score === undefined) return 'var(--ifm-font-color-secondary)';
...
function ScoreBar({ score, max = 10 }) {
  if (score === null) return <span style={{ color: 'var(--ifm-font-color-secondary)', fontSize: '0.8rem' }}>Pendiente</span>;
  const pct = (score / max) * 100;
  return (
    <div className="vsa-eval__bar">
      <div className="vsa-eval__bar-fill" style={{ width: `${pct}%`, background: scoreColor(score) }} />
...
function SectionTags({ sections, small }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem', marginTop: small ? '0.2rem' : '0.4rem' }}>
      {sections.map((s) => (
        <span key={s} style={{
          fontSize: '0.65rem',
          padding: '0.1rem 0.35rem',
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
          <div>
            <h2 style={{ margin: '0 0 0.3rem', fontSize: '1.3rem' }}>{perspective.name}</h2>
            <p style={{ margin: '0 0 0.5rem', color: 'var(--ifm-font-color-secondary)', fontSize: '0.9rem' }}>{perspective.tag}</p>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 120 }}>
            <div style={{ fontSize: '0.75rem', color: 'var(--ifm-font-color-secondary)', marginBottom: '0.25rem', textTransform: 'uppercase', fontWeight: 600 }}>Score</div>
        ...
        {perspective.topValuation && (
          <div style={{
            ...
            <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', fontWeight: 700, color: 'var(--vsa-success)', marginBottom: '0.25rem' }}>Lo mejor</div>
            <p style={{ margin: 0, fontSize: '0.9rem' }}>{perspective.topValuation}</p>
          </div>
        )}
        ...
        {perspective.mainCritique && (
          <div style={{
            ...
            <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', fontWeight: 700, color: 'var(--vsa-danger)', marginBottom: '0.25rem' }}>Critica principal</div>
            <p style={{ margin: 0, fontSize: '0.9rem' }}>{perspective.mainCritique}</p>
          </div>
        )}

        {perspective.sections && perspective.sections.length > 0 && (
          <div style={{ marginBottom: '0.75rem' }}>
            <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', fontWeight: 700, color: 'var(--ifm-font-color-secondary)', marginBottom: '0.4rem' }}>Secciones que evalua</div>
            <SectionTags sections={perspective.sections} />
          </div>
        )}

        {perspective.skillFile && (
          <div style={{ marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid var(--vsa-table-border)' }}>
            <a
              href={`https://github.com/Soboc/venezuela-sa/blob/main/skills/${perspective.skillFile}`}
              target="_blank"
      ...
      <div style={{ overflowX: 'auto' }}>
        <svg viewBox={`0 0 ${width} ${height}`} width="100%" style={{ maxWidth: width, display: 'block', margin: '0 auto' }}>
          ...
          {[6, 7, 8].map((v) => {
            ...
            return (
              <g key={v}>
                <line x1={padL} y1={y} x2={width - padR} y2={y} stroke="var(--vsa-table-border)" strokeWidth="1" strokeDasharray="3,3" />
                <text x={padL - 6} y={y + 4} textAnchor="end" fontSize="10" fill="var(--ifm-font-color-secondary)">{v}</text>
              </g>
            );
          })}
          ...
          {points.map((p, i) => (
            <g key={i}>
              <circle cx={p.x} cy={p.y} r="4" fill="var(--ifm-color-primary)" stroke="var(--ifm-background-color)" strokeWidth="2" />
              <text x={p.x} y={p.y - 10} textAnchor="middle" fontSize="11" fontWeight="700" fill="var(--ifm-heading-color)">{p.score}</text>
              <text x={p.x} y={height - 5} textAnchor="middle" fontSize="9" fill="var(--ifm-font-color-secondary)">{p.label}</text>
              <text x={p.x} y={height - 16} textAnchor="middle" fontSize="8" fill="var(--ifm-font-color-secondary)">{p.perspectives}p</text>
            </g>
          ))}
        </svg>
      </div>
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--ifm-heading-color)' }}>{perspective.name}</div>
          <div style={{ fontSize: '0.8rem', color: 'var(--ifm-font-color-secondary)', marginTop: '0.1rem' }}>{perspective.tag}</div>
        </div>
...
function TabPerspectivas({ onSelectPerspective }) {
  ...
  const filtered = useMemo(() => {
    return PERSPECTIVES.filter((p) => {
      ...
      if (filterSpectrum !== 'all' && p.spectrum !== filterSpectrum) return false;
      if (filterType !== 'all' && p.type !== filterType) return false;
      if (filterScore !== 'all') {
        if (filterScore === 'pending' && p.score !== null) return false;
        if (filterScore === '8+' && (p.score === null || p.score < 8)) return false;
        if (filterScore === '7-8' && (p.score === null || p.score < 7 || p.score >= 8)) return false;
        if (filterScore === '6-7' && (p.score === null || p.score < 6 || p.score >= 7)) return false;
        if (filterScore === '<6' && (p.score === null || p.score >= 6)) return false;
      }
      return true;
    }).sort((a, b) => {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
        <select value={filterSpectrum} onChange={(e) => setFilterSpectrum(e.target.value)} style={selectStyle}>
        ...
        <span style={{ fontSize: '0.8rem', color: 'var(--ifm-font-color-secondary)', alignSelf: 'center' }}>
      ...
      <div style={{
        ...
        {filtered.map((p) => (
          <PerspectiveCard key={p.id} perspective={p} onClick={onSelectPerspective} />
        ))}
      </div>
  ...
  return (
    <div>
      <p style={{ fontSize: '0.9rem', color: 'var(--ifm-font-color-secondary)', marginBottom: '1rem' }}>
        Cada seccion del plan es evaluada por las perspectivas relevantes a su dominio.
        Click en una seccion para ver los evaluadores.
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {summaries.map((s) => (
          <div key={s.section} style={{
            ...
            <button
              type="button"
              onClick={() => setExpanded(expanded === s.section ? null : s.section)}
              style={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <span style={{ fontSize: '1.1rem', color: 'var(--ifm-font-color-secondary)', transition: 'transform 0.2s', transform: expanded === s.section ? 'rotate(90deg)' : 'none' }}>
                  ▶
                </span>
              </div>
            </button>
            ...
            {expanded === s.section && (
              <div style={{ padding: '0 1rem 0.75rem', borderTop: '1px solid var(--vsa-table-border)' }}>
...
function TabMatriz({ onSelectPerspective }) {
  ...
  return (
    <div>
      <p style={{ fontSize: '0.9rem', color: 'var(--ifm-font-color-secondary)', marginBottom: '0.5rem' }}>
        Mapa de calor: filas = perspectivas, columnas = secciones.
        Verde = 8+, azul = 7-8, amarillo = 6-7, rojo = {'<'}6, gris = no evalua.
      </p>
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
        {[
          ...
          <span key={item.label} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem' }}>
      ...
      <div style={{ overflowX: 'auto', paddingBottom: '1rem' }}>
        <table style={{
          ...
          <thead>
            <tr>
              <th style={{ position: 'sticky', left: 0, background: 'var(--ifm-background-color)', zIndex: 2, padding: '0.3rem 0.5rem', textAlign: 'left', minWidth: 130 }}>
                Perspectiva
              </th>
              {allSections.map((sec) => (
                <th key={sec} style={{
              ...
              <th style={{ padding: '0.3rem 0.5rem', textAlign: 'center', fontWeight: 700, fontSize: '0.65rem' }}>Score</th>
            </tr>
          </thead>
          <tbody>
            {scoredPerspectives
                  ...
                  {allSections.map((sec) => {
                    ...
                    return (
                      <td key={sec} style={{
                        ...
                        borderRadius: 4,
                        background: evaluates ? heatColor(p.score) : '#E0E0E020',
      ...
      <p style={{ fontSize: '0.75rem', color: 'var(--ifm-font-color-secondary)', marginTop: '0.5rem' }}>
        17 perfiles de referencia (pendientes de evaluacion) no se muestran en la matriz.
        Click en el nombre de una perspectiva para ver su detalle.
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      <main style={{ maxWidth: 960, margin: '2rem auto', padding: '0 1rem' }}>
        <h1 style={{ marginBottom: '0.3rem' }}>Evaluacion: {TOTAL_PERSPECTIVES} perspectivas</h1>
        <p style={{ color: 'var(--ifm-font-color-secondary)', fontSize: '0.95rem', marginBottom: '1.5rem' }}>
          El plan se evalua continuamente desde {TOTAL_PERSPECTIVES} perspectivas que cubren todo
          el espectro ideologico: desde Milei (libertario) hasta Piketty
          (izquierda), pasando por inversores, ejecutores, estrategas y perfiles
          de referencia reales del mundo financiero y geopolitico.
          ...
          {TABS.map((tab) => (
            <button
              ...
              style={{
                ...
                marginBottom: '-2px',
                color: activeTab === tab.id ? 'var(--ifm-color-primary)' : 'var(--ifm-font-color-secondary)',
        ...
        {activeTab === 'perspectivas' && <TabPerspectivas onSelectPerspective={handleSelectPerspective} />}
        {activeTab === 'secciones' && <TabSecciones onSelectPerspective={handleSelectPerspective} />}
        {activeTab === 'matriz' && <TabMatriz onSelectPerspective={handleSelectPerspective} />}
